...
- 支持桌面与移动端布局。2026-07-20 在已部署页面观察到 88 场竞赛；这是当日观察结果，数据会随每日更新变化。

![分类筛选后的日历](docs/images/calendar-filtered.png)

![移动端日历](docs/images/mobile-calendar.png)
...
数据来自教育部白名单种子、竞赛聚合站、赛事平台及专项来源，并对日期和 URL 进行校验。数据仅供参考，报名资格、时间、规则和链接均应以竞赛官方网站的最新发布为准。

## 添加数据源

1. 在 `crawlers/` 新建抓取模块，返回竞赛记录列表。
2. 为记录提供名称、来源、分类、时间线和 `officialUrl` 等可获得字段。
3. 在 `main.py` 中导入并接入该抓取函数；必要时在 `crawlers/categories.py` 增加分类关键词。
4. 运行测试与爬取流程，检查生成的 `frontend/data.json`。
